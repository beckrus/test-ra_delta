...
from src.exceptions import ParcelNotFoundException, ParcelNotFoundHTTPException, SessionNotFoundException, SessionNotFoundHTTPException
...
async def register_parcel(data: RegisterParcelDTO, db: DBDep, session_id: SessionIdDep, response: Response) -> ParcelIdDTO:
    try:
        if not session_id:
...
async def get_my_parcels(
    ...
    has_delivery_cost: bool | None = Query(default=None, description="Filter by presence of delivery cost")
...
async def get_my_parcel_by_id(parcel_id:int, db: DBDep, session_id: SessionIdDep) -> ResponseParcelDTO:
    try:
